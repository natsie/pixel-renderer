...
import Colour from "./components/Colour.ts";
import { arr, obj, math, type, convertToPixels } from "./utils.ts";
import Renderable, { validateOpts } from "./components/Renderable.ts";
import type { Any, IPixel, IRenderer } from "./types.ts";
import Sprite from "./components/Sprite.js";
import Text from "./components/Text.js";
import Texture from "./components/Texture.js";

const assignOpts = (target: IPixel.Data, options: IPixel.Config) => {
  ...
  if (canvas != null) target.canvas = _v.Canvas(canvas, "canvas");
  if (backgroundColour != null)
    target.backgroundColour = _v.Colour(backgroundColour, "backgroundColour");

  const context = target.canvas.getContext("2d");
  if (context === null) throw new ReferenceError("Failed to acquire rendering context.");
...
const renderFunctions = new Map<typeof Renderable, (state: IRenderer.RenderState) => null>([
  [
    Sprite,
    (state) => {
      const { context, stack } = state;
      const sprite = stack.pop() as Sprite;
      ...
      try {
        ...
        const calcCache: [number, number, number, number, number, number, number, string] = [
        ...
        {
          const _bgSize: IRenderer.StringNumberVector = [...(style["background-size"] || [1, 1])];
          const _bgPos: IRenderer.StringNumberVector = [
          ...
          const bgBounds: [[number, number], [number, number]] = [
          ...
          context.fillStyle = style["background-colour"]?.toHex() || "#00000000";
          context.fillRect(
            ...bgBounds[0],
            bgBounds[1][0] - bgBounds[0][0],
            bgBounds[1][1] - bgBounds[0][1],
          );
          context.drawImage(
            bgImg.source,
            ...bgBounds[0],
...
const pixelDefaultData: IPixel.Data = {
  width: 640,
  height: 360,
...
class PixelRenderer implements IPixel.Instance {
  static DEBUG_MODE = false;

  data: IPixel.Data = Object.assign({}, pixelDefaultData);
  screen: IPixel.Screen = {
    canvas: sharedData.canvas.offscreen,
    context: sharedData.context.offscreen,
    width: 1,
    height: 1,
  };
  renderables = new Set<IRenderer.Renderable>();

  constructor(options: IPixel.Config = {}) {
    ...
    Object.defineProperties(this.screen, {
      ...
      width: {
        ...
        set: (value: number) => (this.screen.canvas.width = value),
      },
      height: {
        get: () => this.screen.canvas.height,
        // biome-ignore lint/suspicious/noAssignInExpressions:
        set: (value: number) => (this.screen.canvas.height = value),
      },
    });
  }

  add(...renderables: (IRenderer.Renderable | IRenderer.Renderable[])[]): PixelRenderer {
    const _r = arr.flat(renderables, 1);
    if (!_r.every((r) => r instanceof Renderable)) throw new TypeError("Invalid renderable.");

    for (let i = 0; i < _r.length; i++) this.renderables.add(_r[i]);
    return this;
  }

  remove(renderable: Renderable): boolean {
  ...
  render(): null {
    ...
    const state: IRenderer.RenderState = {
      canvas,
      context,
      rendered: new Set<Renderable>(),
    ...
    while (state.stack.length > 0) {
      ...
      const rfunc = renderFunctions.get(obj.getProto(rO)?.constructor);
      if (!rfunc) {
...
export { default as Colour } from "./components/Colour.ts";
export { default as Renderable } from "./components/Renderable.ts";
export { default as Sprite } from "./components/Sprite.ts";
export { default as Text } from "./components/Text.ts";
export { default as Texture } from "./components/Texture.ts";
export { default as Ticker } from "./components/Ticker.ts";
